...
// 使用全局配置 global.GVA_CONFIG.Oracle 进行初始化
// 设计好处：
...
// GormOracleByConfig 初始化Oracle数据库通过传入配置
// 设计好处：
// 1. 灵活性：允许传入自定义配置，支持多数据库实例或动态配置场景
// 2. 可测试性：测试时可以传入测试配置，不依赖全局配置
// 3. 代码复用：与 GormOracle() 共用 initOracleDatabase 辅助函数，避免代码重复
...
// 设计模式：提取公共逻辑到辅助函数
// 好处：
// 1. DRY原则：避免 GormOracle 和 GormOracleByConfig 重复实现相同逻辑
...
func initOracleDatabase(m config.Oracle) *gorm.DB {
	...
	// 使用 GORM 打开 Oracle 数据库连接
	// oracle.Open() 使用 dzwvip/gorm-oracle 驱动，这是 GORM 的 Oracle 适配器
	...
	if db, err := gorm.Open(oracle.Open(m.Dsn()), internal.Gorm.Config(general)); err != nil {
		...
		// 注意：Oracle 连接建立成本较高，合理设置空闲连接数可以显著提升性能
		sqlDB.SetMaxIdleConns(m.MaxIdleConns)
		
		// 设置最大打开连接数：同时打开的最大连接数
		// 好处：限制并发连接数，防止数据库连接过多导致资源耗尽
		// 注意：需要根据 Oracle 数据库服务器配置和应用并发量合理设置，避免超过数据库最大连接数限制
