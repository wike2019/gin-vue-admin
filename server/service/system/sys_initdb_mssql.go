// Package system 提供 MSSQL 数据库初始化服务
// 设计思路：
// 1. 使用策略模式：MssqlInitHandler 实现 TypedDBInitHandler 接口，专门处理 MSSQL 数据库的初始化逻辑
// 2. 使用 context 传递状态：通过 context 传递配置和数据库连接，避免全局变量污染
// 3. 职责分离：每个方法只负责一个特定功能，符合单一职责原则
// 4. 幂等性支持：通过检查数据是否已存在，支持重复初始化而不出错
// 5. MSSQL 特性支持：针对 MSSQL 的字符集、数据类型、索引等进行优化
package system
...
// MssqlInitHandler MSSQL 数据库初始化处理器
// 设计原因：使用结构体实现 TypedDBInitHandler 接口，封装 MSSQL 特定的初始化逻辑
// 好处：
// 1. 类型安全：通过结构体方法实现接口，编译时检查，避免运行时错误
// 2. 可扩展：如果需要添加 MSSQL 特定的配置或方法，可以在结构体中扩展
// 3. 可测试：结构体方法易于单元测试，可以 mock 依赖
// 4. 零值可用：空结构体即可使用，无需额外初始化（Go 语言特性）
// 5. 数据库特定优化：可以针对 MSSQL 的特性（如字符集、数据类型映射等）进行优化
type MssqlInitHandler struct{}

// NewMssqlInitHandler 创建 MSSQL 初始化处理器实例
...
// EnsureDB 确保 MSSQL 数据库存在并建立连接
// 设计原因：MSSQL 需要先连接到服务器，然后才能创建数据库或进行操作
// 好处：
// 1. 统一接口：与其他数据库类型的 EnsureDB 方法接口一致，便于多态调用
// 2. 连接管理：统一管理数据库连接，避免连接泄漏
// 3. 错误处理：统一的错误处理机制，任何步骤失败都会返回错误
// 4. 兼容性：针对 MSSQL 的特性进行优化，确保兼容性
//
// 实现细节说明：
// - 类型检查：验证 context 中的数据库类型，确保调用的是正确的 Handler
// - 配置转换：将请求参数转换为 MSSQL 配置结构体
// - 空数据库名处理：如果没有数据库名，跳过初始化（可能是测试场景）
// - DSN 配置：使用空数据库 DSN 连接到服务器，MSSQL 可以在连接后创建数据库
// - 字符串长度限制：设置 DefaultStringSize 为 191，与 MySQL 保持一致，避免索引长度问题
...
func (h MssqlInitHandler) EnsureDB(ctx context.Context, conf *request.InitDB) (next context.Context, err error) {
	...
	// 获取 MSSQL 空数据库连接字符串（不指定数据库名）
	// 设计原因：MSSQL 可以先连接到服务器，然后在连接上执行 CREATE DATABASE 命令
	// 好处：使用空 DSN 连接到服务器，然后可以创建或选择数据库
	dsn := conf.MssqlEmptyDsn()

	// 配置 MSSQL 数据库连接参数
	// 设计原因：针对 MSSQL 的特性进行配置优化
	// 配置说明：
	//   - DSN: 数据源名称，包含数据库连接信息（主机、端口、用户、密码等）
	//   - DefaultStringSize: 191
	//     设计原因：与 MySQL 保持一致，避免字符串字段过长导致的索引问题
	//     好处：
	//     1. 统一性：所有数据库类型使用相同的默认长度，避免混乱
	//     2. 兼容性：较短的字符串长度可以提高兼容性
	//     3. 性能：较短的索引键可以提高索引性能
	mssqlConfig := sqlserver.Config{
		DSN:               dsn, // DSN data source name
		DefaultStringSize: 191, // string 类型字段的默认长度，与 MySQL 保持一致
	}
	...
	//     注意：MSSQL 在迁移时禁用外键约束，迁移完成后再启用，确保数据完整性
...
// 设计原因：MSSQL 的表创建逻辑与其他数据库相同，直接复用通用函数
// 好处：
// 1. 代码复用：避免重复实现相同的逻辑
